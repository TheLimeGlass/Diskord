...
public class EnumClassInfo<E extends Enum<E>> {
	...
	public void register() {
		if (Classes.getExactClassInfo(enumType) == null) {
			if (Diskord.getInstance().getConfig().getBoolean("UseEnglishEnums")) {
				...
				Diskord.getSyntaxData().set("Syntax.Enums." + enumType.getSimpleName() + ".enums", enums);
			}
	...
	private void enumDebug(String message, String codeName, Class<?> clazz) {
		if (codeName.equalsIgnoreCase(Diskord.getInstance().getConfig().getString("syntaxDebug"))) Diskord.debugMessage("&eRegistered syntaxDebug found enum classinfo: " + codeName + " (" + clazz.getCanonicalName() + ")");
		else Diskord.debugMessage("&5Registered Enum '" + codeName + "' with return class " + enumType.getName());
		Diskord.getSyntaxData().set("Syntax.Enums." + clazz.getSimpleName() + ".user", user);
		Syntax.save();
	}
}