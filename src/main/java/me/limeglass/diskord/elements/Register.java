...
public class Register {
	...
	public Register() {
		try {
			...
			File file = (File) method.invoke(Diskord.getInstance());
			...
			for (Enumeration<JarEntry> jarEntry = Stocksaddon.entries(); jarEntry.hasMoreElements();) {
				...
				if (name.startsWith(Diskord.getPackageName()) && name.endsWith(".class")) {
		...
		run : for (Class clazz : classes) {
			if (!clazz.isAnnotationPresent(Disabled.class)) {
				...
				if (clazz.isAnnotationPresent(Patterns.class)) {
					...
					if (syntax == null) Diskord.debugMessage("&cThere was an issue registering the syntax for " + clazz.getName());
				} else {
					continue run;
				}
				...
				if (syntax != null) {
					if (Effect.class.isAssignableFrom(clazz)) {
						...
						Diskord.debugMessage("&5Registered Effect " + clazz.getSimpleName() + " (" + clazz.getCanonicalName() + ") with syntax " + Arrays.toString(syntax));
					} else if (Condition.class.isAssignableFrom(clazz)) {
						Skript.registerCondition(clazz, syntax);
						Diskord.debugMessage("&5Registered Condition " + clazz.getSimpleName() + " (" + clazz.getCanonicalName() + ") with syntax " + Arrays.toString(syntax));
						...
						try {
							...
							Diskord.debugMessage("&5Registered Expression " + type.toString() + " " + clazz.getSimpleName() + " (" + clazz.getCanonicalName() + ") with syntax " + Arrays.toString(syntax));
						} catch (IllegalAccessException | IllegalArgumentException | InstantiationException e) {
							Diskord.consoleMessage("&cFailed to register expression " + clazz.getCanonicalName());
							e.printStackTrace();
						}
					}
				}
	...
	public static void metrics(Metrics metrics) {
		...
		Diskord.debugMessage("Metrics registered!");
	}
