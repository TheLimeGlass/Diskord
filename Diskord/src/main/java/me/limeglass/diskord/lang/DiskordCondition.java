...
public abstract class DiskordCondition extends Condition implements DataChecker {
	...
	public String toString(Event event, boolean debug) {
		...
		if (event == null) {
			Diskcord.debugMessage(getClass().getSimpleName() + " - " + modSyntax);
		} else {
			Arrays.asList(expressions.getExpressions()).stream().forEach(expression->values.add(expression.toString(event, debug)));
			Diskcord.debugMessage(getClass().getSimpleName() + " - " + modSyntax + " (" + event.getEventName() + ")" + " Data: " + Arrays.toString(values.toArray()));
		}
		return Diskcord.getNameplate() + getClass().getSimpleName() + "- Syntax: " + getSyntax();
	}
