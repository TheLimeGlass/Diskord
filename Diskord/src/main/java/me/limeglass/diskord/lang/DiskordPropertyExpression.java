...
public abstract class DiskordPropertyExpression<F, T> extends PropertyExpression<F, T> {
	...
	public String toString(Event event, boolean debug) {
		...
		if (event != null) Diskcord.debugMessage(getClass().getSimpleName() + " - " + modSyntax + " (" + event.getEventName() + ")" + " Data: " + Arrays.toString(values.toArray()));
		return Diskcord.getNameplate() + getClass().getSimpleName() + ": the " + getPropertyName() + " of " + getExpr().toString(event, debug);
	}
	...
	protected Boolean isNull(Event event) {
		...
		if (getExpr().isSingle() && getExpr().getSingle(event) == null) {
			Diskcord.debugMessage("The property expression was null: " + getExpr().toString(event, true));
			return true;
