...
public class EnumClassInfo<E extends Enum<E>> {
	...
	public void register() {
		if (Classes.getExactClassInfo(enumType) == null) {
			if (Diskcord.getInstance().getConfig().getBoolean("UseEnglishEnums")) {
				...
				Diskcord.getSyntaxData().set("Syntax.Enums." + enumType.getSimpleName() + ".enums", enums);
			}
	...
	private void enumDebug(String message, String codeName, Class<?> clazz) {
		if (codeName.equalsIgnoreCase(Diskcord.getInstance().getConfig().getString("syntaxDebug"))) Diskcord.debugMessage("&eRegistered syntaxDebug found enum classinfo: " + codeName + " (" + clazz.getCanonicalName() + ")");
		else Diskcord.debugMessage("&5Registered Enum '" + codeName + "' with return class " + enumType.getName());
		Diskcord.getSyntaxData().set("Syntax.Enums." + clazz.getSimpleName() + ".user", user);
		Syntax.save();
	}
}