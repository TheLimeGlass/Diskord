...
import me.limeglass.diskord.lang.DiskordParser;

public class LangEnumParser<T extends Enum<T>> extends DiskordParser<T> {
	...
	public LangEnumParser(String variableNamePattern, Class<T> clazz) {
		...
		try {
			...
			Diskcord.getSyntaxData().set("Syntax.Enums." + clazz.getSimpleName() + ".names", enumNames);
		} catch (NullPointerException error) {
			Diskcord.consoleMessage("&cThe class: " + clazz.getName() +  " for classinfo name: " + variableNamePattern + " isn't an Enum!");
		}
		Syntax.save();
	}
